...
function incrementGuestGenCount() {
  try {
    localStorage.setItem(GUEST_GEN_KEY, String(getGuestGenCount() + 1));
  } catch { /* ignore */ }
}
function getUserId() {
  try {
...
export default function FlatCustomizerPanel({
  ...
  const handleGenerate = async () => {
    ...
    try {
      const zoneName = activeZone ? activeZone.replace(/_/g, " ") : config.label.toLowerCase();
      const fullPrompt = `${prompt.trim()}, for the ${zoneName}, ${config.aiSuffix}, transparent background, high quality`;
                ...
                >
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      ...
                      if (selectedGalleryId === item.id) setSelectedGalleryId(null);
                    }}
                  >
                    ×
                  </button>
        ...
        <div className="tsc-main-col">
          ...
          <div className="tsc-preview-panel">
            ...
            <div className="tsc-color-section">
              <div className="tsc-color-label">{config.colorLabel}</div>
