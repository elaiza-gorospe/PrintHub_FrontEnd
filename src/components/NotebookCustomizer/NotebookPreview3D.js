...
// Values tuned for notebook_material.glb — adjust oz / sw / sh as needed after
// inspecting the model in-browser.
const ZONE_PLANE = {
  front_cover: { ox: 0, oy: 0.0, oz: 1, ry: 0, sw: 0.72, sh: 0.88 },
  back_cover: { ox: 0, oy: 0.0, oz: -1, ry: Math.PI, sw: 0.72, sh: 0.88 },
};

export default function NotebookPreview3D({ modelPath, zoneDesigns = {} }) {
  ...
  useEffect(() => {
    ...
    new GLTFLoader().load(
      ...
      (err) => {
        ...
        setError(
          "Failed to load 3D model. Place notebook_material.glb in public/models/.",
        );
      },
    );
